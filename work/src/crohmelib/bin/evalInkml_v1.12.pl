...
OR	evalInkml.pl filecheck.inkml
	...
	evalInkml.pl -S listOfFiles.txt [-H|-I]
...
Options : 
	...
	-I : shows individual stat expr per expr
...
if($onlyStat or $individualStat){
	my $refFile;
	if($individualStat){
		print "File      , EMsizeSYMB, MAXDEPTH, SUMBASELINE, DISTCBASELINE\n";
	}
...
#######################################################
#### Max depth of spatial relation (mathML tag, ignoring mrow,mi,mn,mo) ####
#### param 1 :  sub part of MathML tree     			####
#### param 2 :  stat extrated top-down     			####
#### out : hash with recursive metrics (bottom-up)   			####
#######################################################
sub depthSpatRelStat {
	my ($truth, $fatherStat) = @_;
	my $currD = 0;
	my $currB = 0;
	my $subStat;
	my $locStat = {};
	my $locStat2Children = {};
	 $locStat->{maxDepth} = 0;
	 $locStat->{sumBaseline} = 0;
	 $locStat->{allPath} = {};
	my $childCount = 0;
	my $fatherTag = $fatherStat->{fatherTag};
	foreach my $subExp (@{$truth}){
		$childCount++;
		# max diff Path (path generation)
		$locStat2Children->{currPath} = $fatherStat->{currPath};
		$locStat2Children->{fatherTag} = $subExp->{name};
		#print "Father = " .$fatherTag . " c=" . $childCount . " tag=".$subExp->{name}."\n";
		if($fatherTag ~~ ['math']){
			$locStat2Children->{currPath} = $fatherTag;
		}elsif($fatherTag ~~ ['msub','msup','munder'] and $childCount == 2){
			$locStat2Children->{currPath} = $locStat2Children->{currPath}  . $fatherTag;
		}elsif($fatherTag ~~ ['msqrt'] and $childCount == 1){
			$locStat2Children->{currPath} = $locStat2Children->{currPath}  . $fatherTag ;
		}elsif($fatherTag ~~ ['mfrac', 'mroot']){
			$locStat2Children->{currPath} = $locStat2Children->{currPath}  . $fatherTag . $childCount;
		}elsif(($fatherTag eq 'msubsup') and ($childCount == 2)){
			$locStat2Children->{currPath} = $locStat2Children->{currPath}  . "msub";
		}elsif(($fatherTag eq 'msubsup') and ($childCount == 3)){
			$locStat2Children->{currPath} = $locStat2Children->{currPath}  . "msup";
		}elsif($fatherTag ~~ ['munderover']  and $childCount > 1){
			$locStat2Children->{currPath} = $locStat2Children->{currPath}  . $fatherTag . $childCount;
		}	
		#	print " currPath =" . $locStat2Children->{currPath} . "\n";
		#### recurcivity
		$subStat = &depthSpatRelStat($subExp->{sub},$locStat2Children);
		####
		
		# Depth
		if($subExp->{name} ~~ ['math','mrow','mi','mo','mn']){
			$currD = $subStat->{maxDepth};
		}else{
			$currD = $subStat->{maxDepth}+1;
		}
		if($currD > $locStat->{maxDepth}){ # max of all children
			$locStat->{maxDepth} = $currD;
		}
		# baseline
		if($subExp->{name} ~~ ['msubsup','munderover','mfrac', 'mroot']){
			$currB = 2;
		}elsif($subExp->{name} ~~ ['math', 'msub','msup','munder', 'msqrt']){
			$currB = 1;
		}else{
			$currB = 0;
		}
		#print $subExp->{name} . " " . $currB . "\n";
		if($subExp->{name} ~~ ['mi','mn','mo']){ 
			$locStat->{sumBaseline} += 0; # sum of all children
		}else{
			$locStat->{sumBaseline} += $subStat->{sumBaseline} + $currB;
		}
		# max diff Path (path count)
		if($subExp->{name} ~~ ['math']){ # end
			$locStat->{DISTINCTBASELINE} = 0;
			foreach my $p (keys (%{$subStat->{allPath}})){
				#print $p . " = " . $subStat->{allPath}->{$p} . "\n";
				$locStat->{DISTINCTBASELINE}++;
			}
		}elsif($subExp->{name} ~~ ['mi','mo','mn']){ # only count symbols
			$locStat->{allPath}->{$locStat2Children->{currPath}}++;
		}else{
			if($subExp->{name} ~~ ['mfrac','msqrt','mroot']){ # count symbols and sub path
				$locStat->{allPath}->{$locStat2Children->{currPath}}++;
			}
			foreach my $p (keys (%{$subStat->{allPath}})){ # count sub path
				$locStat->{allPath}->{$p} += $subStat->{allPath}->{$p};
			}
		}
	}
...
sub addInkStat {
	...
	$subStat = &depthSpatRelStat($truth->{XML_GT});
	$stat->{MAXDEPTH}->{$subStat->{maxDepth}}++;
	$stat->{sumBaseline}->{$subStat->{sumBaseline}}++;
	$stat->{DISTINCTBASELINE}->{$subStat->{DISTINCTBASELINE}}++;
	$stat->{GT}++;
...
sub showStats {
	...
	print "--------------------------------------------------------\n";	
		...
		print "--------------------------------------------------------\n";
		print "Histogram of max depth (ignoring mrow):\n";
		print "--------------------------------------------------------\n";	
		foreach $lab (sort { $a <=> $b } keys(%{$stats->{MAXDEPTH}})){
			printf "%10.10s | %4d (%5.2f)\n",$lab,$stats->{MAXDEPTH}->{$lab}, $stats->{MAXDEPTH}->{$lab}/$numGT;
		}
		print "--------------------------------------------------------\n";
		print "Histogram of sum of baseline (ignoring mrow):\n";
		print "--------------------------------------------------------\n";	
		foreach $lab (sort { $a <=> $b } keys(%{$stats->{sumBaseline}})){
			printf "%10.10s | %4d (%5.2f)\n",$lab,$stats->{sumBaseline}->{$lab}, $stats->{sumBaseline}->{$lab}/$numGT;
		}
		print "--------------------------------------------------------\n";
		print "Histogram of disctinct baselines (ignoring mrow):\n";
		print "--------------------------------------------------------\n";	
		foreach $lab (sort { $a <=> $b } keys(%{$stats->{DISTINCTBASELINE}})){
			printf "%10.10s | %4d (%5.2f)\n",$lab,$stats->{DISTINCTBASELINE}->{$lab}, $stats->{DISTINCTBASELINE}->{$lab}/$numGT;
		}
		print "--------------------------------------------------------\n";
...
#######################################################
#### Print stat line for one expression	 		####
#### param 1 :  stat hash    			####
#######################################################
# head : EMsizeSYMB, MAXDEPTH, sumBaseline, disctinct baseline
sub printStatLine{
	my $file = @_[0];
	my $stats = @_[1];
	printf "%-25s, %4d, %4d, %4d, %4d \n", $file, (keys(%{$stat->{EMsizeSYMB}}))[0], (keys(%{$stat->{MAXDEPTH}}))[0], (keys(%{$stat->{sumBaseline}}))[0],(keys(%{$stat->{DISTINCTBASELINE}}))[0],;
}
