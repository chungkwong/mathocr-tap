...
from optimizers import adadelta, adadelta_weightnoise, adam, sgd, rmsprop
...
def train(dim_word=100,  # word vector dimensionality
    ...
    valid,valid_uid_list = dataIterator_valid(valid_datasets[0], 
                         worddicts,
    ...
    for eidx in range(max_epochs):
        ...
        for x, y, a in train:
            ...
            if numpy.mod(uidx, sampleFreq) == 0:
                ...
                fpp_sample=open(tmpdir+'/valid_decode_result.txt','w')
                valid_count_idx=0
                # FIXME: random selection?
                for x,y in valid:
                    for xx in x:
            ...
            if numpy.mod(uidx, validFreq) == 0:
                ...
                # compute wer
                os.system('python compute-wer.py '+tmpdir+'/valid_decode_result.txt ' + os.path.join(valid_datasets[0],"caption.txt") +' '+tmpdir+'/valid.wer')
                fpp=open(tmpdir+'/valid.wer') # %WER 31.63
                stuff=fpp.readlines()
                fpp.close()
                m=re.search('WER (.*)\n',stuff[0])
